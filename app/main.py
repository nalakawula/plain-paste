...
async def lifespan(app: FastAPI):
    ...
    scheduler.add_job(func=crud.delete_expired_pastes, trigger='interval', seconds=10)
    scheduler.start()
    yield
...
async def create_paste(request: Request, content: str = Form(...), burn_after_reading: bool = Form(False)):
    paste = schemas.PasteCreate(content=content, burn_after_reading=burn_after_reading)
